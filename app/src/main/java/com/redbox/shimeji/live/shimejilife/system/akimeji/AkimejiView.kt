...
import com.redbox.shimeji.live.shimejilife.system.akimeji.animations.Animation
import timber.log.Timber
import java.util.*
import java.util.concurrent.atomic.AtomicLong
import kotlin.concurrent.timerTask

class AkimejiView(internal val mContext: Context, var mascotId: Int) : SurfaceView(mContext), SurfaceHolder.Callback {

    // Use fully-qualified ServiceLocator to avoid resolution issues in some toolchains
    val spritesService = com.redbox.shimeji.live.shimejilife.di.ServiceLocator.spritesServiceAkimejiT2

    val helper = com.redbox.shimeji.live.shimejilife.di.ServiceLocator.helperT2
    ...
    internal var gestureListener: GestureDetector.OnGestureListener =
        object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onDoubleTap(e: MotionEvent): Boolean {
                try {
                    ...
                    if (helper.getReappearDelayMs(mContext).toLong() >= 1) {
                        timer.schedule(unused, helper.getReappearDelayMs(mContext).toLong())
                    } else {
    ...
    internal val handler1 = Handler()
    var height1: Int?
    internal val id = NEXT_ID.getAndIncrement()
    internal var isVisible = true
    internal var shimeji: Akimeji
    internal val paint: Paint
    internal var playground: Playground
    internal val scroller: Scroller
    internal val sizeMultiplier: Double

    // private val sizeMultiplier: Double
    internal val speedMultiplier: Double

    //internal val spritesService: SpritesServiceAkimejiT2
    var width1: Int


    init {
        // this.spritesService = SpritesServiceAkimejiT2.getInstance()
        this.paint = Paint()
        ...
        this.shimeji.initialize(frames, this.speedMultiplier, this.playground)
        this.shimeji.startAnimation()
        holder.setFormat(PixelFormat.TRANSPARENT)
        holder.addCallback(this)
        this.gestureDetector = GestureDetector(this.mContext, this.gestureListener)
        this.scroller = Scroller(this.mContext)
        ...
        if (this.isVisible) {
            ...
            if (canvas != null) {
                ...
                if (this.shimeji.isFacingLeft) {
                    // When facing left, flip horizontally so the sprite faces left
                    this.flipHorizontalMatrix.setScale(-1.0f, 1.0f)
                    this.width1.toFloat().let { this.flipHorizontalMatrix.postTranslate(it, 0.0f) }
